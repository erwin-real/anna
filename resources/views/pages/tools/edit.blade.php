...
@section('content')
    ...
    <div class="body-right">
        <div class="container-fluid">
            <h1 class="h2 mb-0 text-gray-800">{{'Update Tool'}}</h1>
            ...
            <div class="container-fluid mt-5 col-lg-10 col-sm-10">
                <div class="card shadow mb-4">
                    <div class="card-header">{{ __('Update Tool\'s Information') }}</div>

                    <div class="card-body">

                        <form action="{{ action('ToolController@update', $tool->id) }}" method="POST" onsubmit="return check()">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-12">
                                    <input id="name" type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" value="{{$tool->name}}" name="name" required autofocus>
                                ...
                                <table class="table table-bordered" id="materialsTable" width="100%" cellspacing="0">
                                    ...
                                    <tbody id="materialsBody">
                                        @foreach($tool->singleTools as $singleTool)
                                            <tr>
                                                <td><input type="text" class="form-control" name="names[]" value="{{$singleTool->name}}" required></td>
                                                <td><input type="number" class="form-control" name="numbers[]" value="{{$singleTool->number}}" required></td>
                                                <td>
                                                    <select name="periods[]" id="" class="form-control select-class">
                                                        <option value="days" {{$singleTool->period == 'days' ? 'selected' : ''}}>days</option>
                                                        <option value="weeks" {{$singleTool->period == 'weeks' ? 'selected' : ''}}>weeks</option>
                                                        <option value="months" {{$singleTool->period == 'months' ? 'selected' : ''}}>months</option>
                                                        <option value="years" {{$singleTool->period == 'years' ? 'selected' : ''}}>years</option>
                                                    </select>
                                                </td>
                                                <td class="text-center">
                                                    <i class="fa fa-trash del-btn" style="cursor: pointer;" onclick="deleteRow(this)"></i>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
